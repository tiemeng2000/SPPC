 ...
 * 2015-03-24
 *
 * 增加：1、双手pinch触发resize
        2、pinch三状态变量
        3、
        
 * 改变：1、mousemove中判断flag_pinch状态
 *      2、pinch中判断pinch状态
 *      3、
 */
...
(function($){
    ...
    var NOT_PINCHED = 0;            //pinch状态常量->未pinch
    var SINGLE_PINCHED = 1;         //pinch状态常量->单手pinch
    var DOUBLE_PINCHED = 2;         //pinch状态常量->双手pinch
    var flag_pinch = NOT_PINCHED;   //pinch状态           
    ...
    $.updateElementSize = function (eventElement, currCursorPosition, prevCursorPosition){
        ...
        //更新eventElement位置
        $(eventElement).css("height", ($(eventElement).prop("clientHeight") - spanY) );
        $(eventElement).css("top", ($(eventElement).prop("offsetTop") + spanY/2) );
        $(eventElement).css("width", ($(eventElement).prop("clientWidth") - spanX) );
        $(eventElement).css("left", ($(eventElement).prop("offsetLeft") + spanX/2) );
    }
