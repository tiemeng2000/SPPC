 ...
 * 2015-03-31
 *
 * 增加：1、Leap.Controller的option变量
        2、gesture遍历: frame.gestures.forEach()
        
 * 改变：1、Element的css, SVG的用attr
 *      2、controller.on()增加gesture形参
 *      3、去掉gTriggerEvent_Pinch的frame参数
 */
...
(function($){
    ...
    //连接Leap设备
    $.gInit = function (){
        //controller的Arguments
        //var option = {enableGestures: true};
        ...
        controller.on('frame', function (frame){
            ...
            frame.hands.forEach ( function (hand, index){
                cursor = ( cursors[index] || (cursors[index]=new $.gCursor()) );    //通过index 关联 cursor
                g_position = ( g_positions[index] || (g_positions[index]=[0,0]) );  //通过index 关联 g_position
            ...
            frame.gestures.forEach( function (gesture){
                ...
                //触发"Tap"
                $.gTriggerEvent_Tap(cursor, currPosition);
            });
        }); 
    ...
    $.updateElementPosition = function (eventElement, currCursorPosition, prevCursorPosition){
        ...
        $(eventElement).attr("cx", ($(eventElement).attr("cx")-spanX) );
        $(eventElement).attr("cy", ($(eventElement).attr("cy")-spanY) );
    }
    ...
    $.gTriggerEvent_Hover = function (visualCursor, prevCursorPosition, currCursorPosition){
        ...
        // if(eleCurr.tagName==="svg"){
        // console.log(eleCurr);}
    }

    //Pinch捏取事件响应函数（绑定 MouseDown 和 MouseUp ）
    $.gTriggerEvent_Pinch = function (visualCursor, currCursorPosition, hand){
        var pinchThreshold = 40;    //指尖距离40作为pinch发生的阈值
        var tipDist = $.distanceOfThumbIndex(hand);  //获取指尖距离
    ...
    $.gTriggerEvent_Tap = function (visualCursor, currCursorPosition){
        //创建Tap事件, 模拟click事件
        var e_click = $.Event('click');
        e_click.pageX = (currCursorPosition === null) ? 0 : currCursorPosition[0];
        e_click.pageY = (currCursorPosition === null) ? 0 : currCursorPosition[1];

        //获取cursor位置元素
        visualCursor.hide();
        var element = document.elementFromPoint(e_click.pageX, e_click.pageY);  //当前位置元素
        visualCursor.show();

        $(element).trigger(e_click);  //在element上触发mousemove事件
    }


    //拇指食指指尖距离计算
    $.distanceOfThumbIndex = function (hand)
    {
        var d = 0;  //指尖距离
