/*
 * 2016-02-15 *
 * 增加：1、cursor.css的手势关联，pinch时改为pinchcursor
 *      2、handPinchCheck中，pinchStrength触发pinch状态
 *      3、distanceOfThumbIndex中，增加用leap自带计算距离函数计算dI
 *		4、leapToScreen中，增加doc的h、w
 *
 * 改变：1、cursor的原点改为手型图标(待进一步设计)
 *      2、leapToScene的备注从tip改为hand
 *      3、handPinchCheck中用pinchStrength触发pinch状态，pinchThreshold设置为0.6
 */
...
(function($){
    ...
    $.LeapG.Controller = function(){
        ...
        this.onGesture = function (gesture,hand){
            ...
            if (type === "swipe" && state === "start") {
                //gesture发生的手指id
                if(hand.pointables[4]){
                    if(parseInt(pointableIds) === hand.pointables[4].id){
    ...
    $.LeapG.Pointer = function (){
        ...
        this._createCursor = function(){
            ...
            this.cursor.css({           //细化cursor样式
                display: 'block',
                'background-image': 'url(../leapjs-MouseHands/css/RightHand_pinch.png)',
                opacity: '1',
                borderRadius: '0px',
                margin: '0px',
                padding: '0px',
        ...
        this.bindPointerToHand = function(index, handId){
            this.index = index;
            this.handId = handId;
            console.log("this.index: "+this.index +" "+" this.handId: "+ this.handId);
        }
        ...
        this.onHandPinch = function(){
            this.cursor.css({'background-image': 'url(../leapjs-MouseHands/css/RightHand_fist.png)'});
            this.tips = $("<span>Pinching</span>");
            this.tips.css({'margin-top':'15px', 'position':'absolute', 'color':'#9966FF'});
        ...
        this.onHandUnpinch = function(){
            this.cursor.css({'background-image': 'url(../leapjs-MouseHands/css/RightHand_pinch.png)'});
            this.tips.remove();
    ...
    $.LeapG.leapToScene = function (hand, frame){
        ...
        var pipThumbStabliezed = hand.fingers[0].stabilizedTipPosition;     //拇指稳定位置
        ...
        //********************************************
        //hand位置xy取拇指，z去食指
        var tipx = xPalm; //用palm的x坐标作为cursor的x
        var tipy = yPalm; //用palm的Y坐标作为cursor的y
        var tipz = zPalm;
